...
(function($){
	...
	customTemplate.prototype.renderMessage = function (msgData) {
		var messageHtml = '';
		if (msgData && msgData.type === "events") {
			console.log("msgData.message.type",msgData.message.type)
			if(msgData.message.type === "typing"){
				if(msgData.message.author.profImage == "no-avatar"){
					$('.typingIndicator').css('background-image', "url(" + agentIcon + ")");
				}else{
					$('.typingIndicator').css('background-image', "url(" + msgData.message.author.profImage + ")");
				}
				
				$('.typingIndicatorContent').css('display', 'block');
			}else if(msgData.message.type === "stoptyping"){
				$('.typingIndicatorContent').css('display', 'none');
			}
			// messageHtml = $(this.getChatTemplate("agentConnectedTemplate")).tmpl({
			// 	'msgData': msgData,
...
print(JSON.stringify(message)); */
	...
	<li {{if msgData.type !== "bot_response"}}id="msg_${msgItem.clientMessageId}"{{/if}} class="{{if msgData.type === "bot_response"}}fromOtherUsers{{else}}fromCurrentUser{{/if}} {{if msgData.icon}}with-icon{{/if}}"> \
			<div class = "listTmplContent advancedMultiSelect"> \
				{{if msgData.createdOn && !msgData.message[0].component.payload.sliderView}}<div aria-live="off" class="extra-info hide">${helpers.formatDate(msgData.createdOn)}</div>{{/if}} \
				{{if msgData.icon}}<div aria-live="off" class="profile-photo"> <div class="user-account avtar" style="background-image:url(${msgData.icon})"></div> </div> {{/if}} \
...
var advancedListTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
	...
	{{if msgData && msgData.createdOn}}<div class="extra-info hide">${helpers.formatDate(msgData.createdOn)}</div>{{/if}} \
	...
	var agentConnectedTemplate = '<script id="chat_message_tmpl" type="text/x-jqury-tmpl"> \
	{{if msgData.message}} \
		{{each(key, msgItem) msgData.message}} \
			<li data-time="${msgData.createdOnTimemillis}" id="${msgData.messageId || msgItem.clientMessageId}" class="agentIdentity"> \
				<div><span class="simpleMsg" {{if msgData}}msgData="${JSON.stringify(msgData)}" {{/if}}>{{html helpers.convertMDtoHTML(msgItem.component.payload.text, "bot",msgItem)}}</span> \</div> \
			</li> \
		{{/each}} \
	{{/if}} \
</scipt>';
		...
		}else if(tempType === "agentConnectedTemplate"){
			return agentConnectedTemplate;
		} else {
			return "";
		}
		return "";
