(function (factory) {
    ...
    return function koreBotChat() {
        ...
        return (function ($, KRPerfectScrollbar) {
            ...
            chatWindow.prototype.bindEvents = function () {
                ...
                // _chatContainer.on('focus', '.chatInputBox', function (event) {
                //         var isAgentConnectedFlag = localStorage.getItem("agentConnected");
                ...
                _chatContainer.off('click', '.buttonTmplContentBox li,.listTmplContentChild .buyBtn,.viewMoreList .viewMore,.listItemPath,.quickReply,.carouselImageContent,.listRightContent,.checkboxBtn,.likeDislikeDiv,.buttonQuickReply').on('click', '.buttonTmplContentBox li,.listTmplContentChild .buyBtn, .viewMoreList .viewMore,.listItemPath,.quickReply,.carouselImageContent,.listRightContent,.checkboxBtn,.likeDislikeDiv,.buttonQuickReply', function (e) {
                    ...
                    if (type == "postback" || type == "text") {
                        ...
                        $(parentElement).css('pointer-events', 'none');
                        ...
                        if ($(this).attr('msgData') !== undefined) {
                            ...
                            if ((msgData && msgData.message && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.buttons && msgData.message[0].component.payload.buttons[0] && msgData.message[0].component.payload.buttons[0].title && msgData.message[0].component.payload.buttons[0].title === "Login") || (msgData && msgData.message && msgData.message[0].component && (msgData.message[0].component.formData || (msgData.message[0].component.payload && msgData.message[0].component.payload.formData)))) {
                                if(msgData && msgData.message && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.buttons && msgData.message[0].component.payload.buttons[0] && msgData.message[0].component.payload.buttons[0].title && msgData.message[0].component.payload.buttons[0].title === "Login"){
                                    msgData.message[0].component.payload.formData = {
                                        "formLink": msgData.message[0].component.payload.buttons[0].url,
                                        "renderType": "pop-up"
                                    }
                                    me.renderWebForm(msgData);
                                    $(this).hide();
                        ...
                        var a_link =$(this).attr('url')|| $(this).attr('value');
                ...
                bot.on("message", function (message) {
                    ...
                    if (tempData.from === "bot" && tempData.type === "events" && tempData.message.type === "typing") {
                            console.log("msgData.message.type",tempData.message.type)
                                if(tempData.message.author.profImage == "no-avatar"){
                                    if(KoreSDK.chatConfig.agentImage){
                                        $('.typingIndicator').css('background-image', "url(" + KoreSDK.chatConfig.agentImage + ")");
                                    }else{
                                        $('.typingIndicator').css('background-image', "url(" + agentIcon + ")");
                                    }
                                    
                                }else{
                                    $('.typingIndicator').css('background-image', "url(" + tempData.message.author.profImage + ")");
                                }
            ...
            chatWindow.prototype.sendMessage = function (chatInput, renderMsg, msgObject) {
                ...
                attachmentInfo = {};
                $('.kore-chat-body ul li:last').css('pointer-events', 'none');
                bot.sendMessage(messageToBot, function messageSent(err) {
                    if (err && err.message) {
                        setTimeout(function () {
                            $('.kore-chat-window [data-time="' + clientMessageId + '"]').find('.messageBubble').append('<div class="errorMsg">Your session expired. Please refresh to start again</div>');
                        }, 350);
                    }
                });
                chatInput.html("");
                ...
                _bodyContainer.css('bottom', _footerContainer.outerHeight());
                resetPingMessage();
                var agentConnected = localStorage.getItem("agentConnected");
                if(!agentConnected || agentConnected === "false"){
                    $('.typingIndicatorContent').css('display', 'block');
                    setTimeout(function () {
                        $('.typingIndicatorContent').css('display', 'none');
                    }, 10000);
            ...
            chatWindow.prototype.renderMessage = function (msgData) {
                ...
                if (msgData.type === "bot_response") {
                    ...
                    setTimeout(function () {
                        if ((msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "live_agent") || (msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "SYSTEM" && msgData.message[0].component.payload.text.indexOf("connected") > -1) ) {
                            $('.typingIndicator').css('background-image', "url(" + agentIcon + ")");
                        }else{
                ...
                if (msgData.message && msgData.message[0] && msgData.message[0].cInfo && msgData.message[0].cInfo.body === "Login Form is successfully submitted." || msgData.message && msgData.message[0] && msgData.message[0].cInfo && msgData.message[0].cInfo.body === "UserDetails Form is successfully submitted.") {
                    return false;
                }
                ...
                if (messageHtml === '' && msgData && msgData.message && msgData.message[0]) {
                    ...
                    if(msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.text && isValidJSONString(msgData.message[0].component.payload.text)){
                        //usage
                        var personJSONString = msgData.message[0].component.payload.text;
                        if(isValidJSONString(personJSONString)){
                            //cool we are valid, lets parse
                            var person= JSON.parse(personJSONString);
                            msgData.message[0].component = person;
                            return me.renderMessage(msgData);
                        }
                        ...
                        if(msgData.message[0].component && msgData.message[0].component.payload){
                            if(msgData && msgData.author && msgData.author.profImage){
                                KoreSDK.chatConfig.agentImage = msgData.author.profImage;
                            }
                ...
                if (msgData.type === "bot_response"){
                    if(msgData.message && msgData.message[0] && msgData.message[0].cInfo && msgData.message[0].cInfo.body && msgData.message[0].cInfo.body.includes("Please provide a valid message")){
                        return;
                    }
                }
                                    //For Agent presence
            ...
            chatWindow.prototype.getChatTemplate = function (tempType) {
                ...
                var chatWindowTemplate = '<script id="chat_window_tmpl" type="text/x-jqury-tmpl"> \
                    <div class="kr-sdkMainContainer no-show">\
                    <div class="kore-chat-window droppable liteTheme-one"> \
                    <div class="kr-wiz-menu-chat defaultTheme-kore">\
                    </div>	\
                        <div class="minimized"><span class="messages">Ask Citi</span></div> \
                        <div class="kore-chat-header"> \
                            ...
                            <div class="chat-box-controls"> \
                                ...
                                <button class="close-chat-btn" style="display: none;">\
                                    <img src="data:image/png;base64,">\
                                </button>\
                                </div> \
                                ...
                                <div class="fa fa-chevron-left quickreplyLeftIcon hide"></div><div class="fa fa-chevron-right quickreplyRightIcon hide"></div>\
                                    <div class="quick_replies_btn_parent"><div class="autoWidth">\
                                        ...
                                        {{if msgData.message[0].component.payload.buttons && msgData.message[0].component.payload.buttons.length}} \
                                            {{each(key, msgItem) msgData.message[0].component.payload.buttons}} \
                                                <div class="buttonTmplContentChild quickReplyDiv displayInline"> <span actual-value="${msgItem.title}" {{if msgData}}msgData="${JSON.stringify(msgData)}"{{/if}} {{if msgItem.payload}}value="${msgItem.payload}"{{/if}} class="buttonQuickReply {{if msgItem.image_url}}with-img{{/if}}" type="${msgItem.type}" {{if msgItem.url}}url="${msgItem.url}"{{/if}}>\
                                                    {{if msgItem.image_url}}<img src="${msgItem.image_url}">{{/if}} <span class="quickreplyText {{if msgItem.image_url}}with-img{{/if}}">${msgItem.title}</span></span>\
                                                </div> \
                                            {{/each}} \
                                        {{/if}} \
            ...
            this.openMinimisedChatWindow = function (cfg) {
                if ($(".kore-chat-window").hasClass('minimize')) {
                    $(".kore-chat-window").removeClass('minimize');
                } else {
                    $(".kore-chat-window").addClass('minimize');
                }
            ...
            this.hideChatWindow = function (cfg) {
                $(".kore-chat-window").hide();
            }

            this.isChatWindowMinimized = function (cfg) {
                if ($(".kore-chat-window").hasClass('minimize')) {
                    return true;
                } else {
                    return false;
            ...
            this.botDetails = function (response, botInfo) {
                if(window.AgentDesktop){
                    console.log("AgentDesktop initialization1 - did not receive authResponse", JSON.stringify(response))
                    if (response && response.userInfo) {
                        AgentDesktop(response.userInfo.userId, response);
                    } else {
                        console.log("AgentDesktop initialization2 - did not receive authResponse")
                    }    
                }else{
                    console.log("AgentDesktop initialization3 - did not receive authResponse")
                }
                // if(window.AgentDesktop){
                //     if (botInfo && botInfo.userInfo) {
                //         AgentDesktop(botInfo.userInfo.userId, botInfo);
                //     } else {
                ...
                //             $(".logo-icon").css("background-image", "url(https://www.citi.com/CBOL/IA/Angular/assets/citilogoredesign.png)");
            ...
            ************************************************************************************************************************************************/
            ...
            return {
                ...
                openMinimisedChatWindow: openMinimisedChatWindow
            };
